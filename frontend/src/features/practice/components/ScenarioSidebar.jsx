...
const ScenarioSidebar = ({ scenario, guidance, character = null, live2DStatus = "idle", lipSyncLevel = 0 }) => {
          ...
          <ol className="max-h-[190px] space-y-3 overflow-y-auto pr-1 lg:mt-5 lg:max-h-none lg:space-y-4 lg:overflow-visible lg:pr-0">
            {focusItems.map((item, index) => (
              <li key={`${item}-${index}`} className="flex items-start gap-2 lg:gap-3">
                <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--page-subtle)] lg:mt-2" />
                <p className="text-xs leading-snug text-[var(--page-fg)] lg:text-sm">{item}</p>
              </li>
            ))}
          </ol>
        </div>
