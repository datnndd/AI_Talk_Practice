  ...
  return (
    <div className="flex min-h-screen items-center justify-center app-page-shell px-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-[28px] border border-border bg-card p-8 shadow-xl">
        <h1 className="text-2xl font-black text-[var(--page-fg)]">Đặt lại mật khẩu</h1>
        <p className="mt-2 text-sm font-medium text-[var(--page-muted)]">Nhập OTP đã gửi qua email và mật khẩu mới.</p>
        ...
        <div className="mt-6 space-y-4">
          <input value={email} onChange={(event) => setEmail(event.target.value)} required type="email" className="h-12 w-full rounded-xl border border-border bg-card px-4 font-semibold" placeholder="Email" />
          <input value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, "").slice(0, 6))} required minLength={6} maxLength={6} inputMode="numeric" className="h-12 w-full rounded-xl border border-border bg-card px-4 text-center font-semibold tracking-[0.35em]" placeholder="OTP" />
          <input value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required minLength={6} type="password" className="h-12 w-full rounded-xl border border-border bg-card px-4 font-semibold" placeholder="Mật khẩu mới" />
          <input value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} required minLength={6} type="password" className="h-12 w-full rounded-xl border border-border bg-card px-4 font-semibold" placeholder="Nhập lại mật khẩu mới" />
        </div>
