...
from app.modules.sessions.services.hybrid_conversation.evaluation import NoOpEvaluationHook, PostTurnEvaluationHook
...
class DialogueOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        llm_clients: ConversationLLMClients | None = None,
        ...
        enable_llm_relevance_analysis: bool | None = None,
        enable_llm_fact_extraction: bool | None = None,
        evaluator: PostTurnEvaluationHook | None = None,
    ):
        self.scenario = scenario
        if llm_clients is None:
            if llm is None:
                raise ValueError("DialogueOrchestrator requires llm or llm_clients")
            llm_clients = ConversationLLMClients(analysis=llm, dialogue=llm, evaluation=llm)
        self.llm_clients = llm_clients
        self.llm = llm_clients.dialogue
        ...
        self.analysis_service = LLMTurnAnalysisService(
            llm=llm_clients.analysis,
            analyzer=self.analyzer,
            enable_llm_relevance=(
                settings.conversation_enable_llm_relevance_analysis
                if enable_llm_relevance_analysis is None
                else enable_llm_relevance_analysis
            ),
            enable_llm_fact_extraction=(
                settings.conversation_enable_llm_fact_extraction
                if enable_llm_fact_extraction is None
                else enable_llm_fact_extraction
            ),
            max_tokens=settings.analysis_llm_max_tokens or 360,
        )
    ...
    def create(
        ...
        *,
        ...
        return cls(
            ...
            enable_llm_relevance_analysis=enable_llm_relevance_analysis,
            enable_llm_fact_extraction=enable_llm_fact_extraction,
        )
    ...
    async def stream_turn(self, user_text: str) -> AsyncGenerator[str, None]:
        ...
        analysis, facts = await self.analysis_service.analyze(
            user_text,
        ...
        if action.action == PolicyAction.GENERATE:
            ...
            async for chunk in self.llm.chat_stream(
                ...
                max_tokens=settings.dialogue_llm_max_tokens or settings.llm_max_tokens,
            ):
