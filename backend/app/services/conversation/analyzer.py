...
from app.services.conversation.schemas import DialogueState, MemoryFact, ScenarioDefinition, TurnAnalysis, TurnLabel
...
class TopicAnalyzer:
    ...
    def analyze(
        ...
        *,
        ...
        elif self._is_too_short(clean):
            labels.append(TurnLabel.TOO_SHORT)

        scenario_tokens = self._scenario_tokens(scenario, state)
        turn_tokens = _tokens(clean)
        relevance = self._relevance_score(turn_tokens, scenario_tokens, clean)
        contains_fact = bool(extracted_facts)
        contains_direct_answer = relevance >= self.on_topic_threshold or self._has_direct_answer_signal(clean)
    ...
    def _relevance_score(self, turn_tokens: set[str], scenario_tokens: set[str], clean: str) -> float:
        if not clean:
            return 0.0
