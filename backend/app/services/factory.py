...
def create_asr(config: Settings) -> ASRBase:
    ...
    if provider == "dashscope":
        from app.services.asr.dashscope_asr import DashScopeASR
        logger.info("Using DashScope ASR (cloud API)")
        return DashScopeASR(config)

    elif provider == "faster_whisper":
        from app.services.asr.faster_whisper_asr import FasterWhisperASR
...
def create_tts(config: Settings) -> TTSBase:
    ...
    if provider == "dashscope":
        from app.services.tts.dashscope_tts import DashScopeTTS
